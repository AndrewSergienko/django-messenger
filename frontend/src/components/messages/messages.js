	...
	render() {
		...
		return (
			<Wrap>
						...
						return (
							<Message key={index}>
								<Avatar
									...
									onClick={() => {
										this.openModal();
										this.setState({ userId: message.user });
									}}
								/>
								<section>
									<Username>
										{friend.id === message.user ? `${friend.first_name} ${friend.last_name}` : `${me.first_name} ${me.last_name}`}
									</Username>
									<Time>
				...
				{activeChat ? (
					<form onSubmit={event => this.onSubmit(event, activeChat)}>
						<Input
							type="text"
