...
export default class Chat extends Component {
	...
	chatCreatedNotify = data =>
		data.message &&
		toast.info(
			<NotifyTitle>
				{data.message.user.first_name} {data.message.user.last_name} was created chat with you.
			</NotifyTitle>,
			{
				position: "top-right",
				autoClose: 5000,
	...
	changeUserInfo = async (first_name, last_name) => {
		const result = await this.server.changeUserProfile(first_name, last_name, this.props.authToken);
		this.setState({ me: result });
	};
	...
	render() {
		...
		return (
			<>
				...
				<ChatSideBar chats={this.state.chats} userInfo={this.userInfo} chatMessages={this.chatMessages} setActiveChat={this.setActiveChat} />
				{this.state.activeChat ? (
					...
					<ChooseChat>Select a chat to start a messaging</ChooseChat>
				)}
				<StyledNotify position="top-right" autoClose={5000} newestOnTop closeOnClick rtl={false} pauseOnFocusLoss draggable pauseOnHover />
			</>
		);
	}
}
